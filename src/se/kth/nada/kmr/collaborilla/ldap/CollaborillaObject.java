...
public class CollaborillaObject extends LDAPObject implements Cloneable {
     ...
     */
    public CollaborillaObject(LDAPAccess dir, String serverDN, String uri,
            boolean create) throws LDAPException {
        this.ldapAccess = dir;
        ...
        if (!this.entryExists()) {
            if (create) {
                this.createEntryWithContainer(LDAPStringHelper
                        .dnToParentDN(this.baseDN),
                        CollaborillaObjectConstants.OBJECTCLASS,
                        CollaborillaObjectConstants.INFONODETYPE,
                ...
                throw new LDAPException(
                        ...
                        LDAPException
                                .resultCodeToString(LDAPException.NO_SUCH_OBJECT),
                        this.baseDN);
            }
        }
    }
    ...
    private void updateBaseDN() {
        String tmpDN = CollaborillaObjectConstants.INFONODETYPE
                ...
                + ","
                + LDAPStringHelper
                        .uriToBaseDN(CollaborillaObjectConstants.ROOT,
                                this.serverDN, this.uri,
                                CollaborillaObjectConstants.INFOCONTAINERTYPE);

        if (this.revision > 0) {
            tmpDN = CollaborillaObjectConstants.INFONODETYPE + "="
                    + this.revision + "," + tmpDN;
        }
    ...
    public void setRevision(int rev) throws LDAPException {
        ...
        if (!this.entryExists()) {
            ...
            throw new LDAPException("NO SUCH OBJECT",
                    LDAPException.NO_SUCH_OBJECT, LDAPException
                            .resultCodeToString(LDAPException.NO_SUCH_OBJECT),
                    tmpDN);
        }
    }
    ...
    public int getRevisionCount() throws LDAPException {
        if (this.revision == 0) {
            ...
            return this.childCount(LDAPStringHelper.dnToParentDN(this.baseDN),
    ...
    public void createRevision() throws LDAPException {
        ...
        String destDN = CollaborillaObjectConstants.INFONODETYPE + "="
                + revisionNumber + "," + this.baseDN;
    ...
    public String getRevisionDN(int rev) {
        if (rev == 0) {
            ...
            return CollaborillaObjectConstants.INFONODETYPE + "=" + rev + ","
                    + this.baseDN;
        }
    }
    ...
    public void restoreRevision(int rev) throws LDAPException {
        if (rev > this.getRevisionCount()) {
            throw new LDAPException("Revision does not exist",
                    LDAPException.NO_SUCH_OBJECT, "Revision does not exist");
        }
        ...
        if (result == null) {
            ...
            while ((parentURI = LDAPStringHelper.getParentURI(parentURI)) != null) {
                try {
                    ...
                    result = this
                            .readAttribute(CollaborillaObjectConstants.LOCATION);
                } catch (LDAPException e) {
                ...
                if (result != null) {
                    String append = originalURI.substring(parentURI.length(),
                            originalURI.length());

                    for (int i = 0; i < result.length; i++) {
                        if (result[i].endsWith("/") && append.startsWith("/")) {
                            result[i] = result[i].substring(0, result[i]
                                    .length() - 1);
                        }
            ...
            throw new LDAPException("NO SUCH ATTRIBUTE",
                    LDAPException.NO_SUCH_ATTRIBUTE,
                    "Unable to construct a URL from parent entries",
                    this.baseDN);
        }
    ...
    public void modifyLocation(String oldUrl, String newUrl)
            throws LDAPException {
        this.modifyAttribute(CollaborillaObjectConstants.LOCATION, oldUrl,
                newUrl);
    }
    ...
    public void modifyUriOriginal(String oldUri, String newUri)
            throws LDAPException {
        this.modifyAttribute(CollaborillaObjectConstants.URIORIG, oldUri,
                newUri);
    }
    ...
    public void modifyUriOther(String oldUri, String newUri)
            throws LDAPException {
        this.modifyAttribute(CollaborillaObjectConstants.URIOTHER, oldUri,
                newUri);
    }
    ...
    public String getContextRdfInfo() throws LDAPException {
        if (this.attributeExists(CollaborillaObjectConstants.CONTEXTRDFINFO)) {
            return this
                    .readAttribute(CollaborillaObjectConstants.CONTEXTRDFINFO)[0];
        } else {
            return null;
        }
    }
    ...
    public void setContextRdfInfo(String rdfInfo) throws LDAPException {
        if (this.attributeExists(CollaborillaObjectConstants.CONTEXTRDFINFO)) {
            this.resetAttribute(CollaborillaObjectConstants.CONTEXTRDFINFO,
                    rdfInfo);
        } else {
            this.addAttribute(CollaborillaObjectConstants.CONTEXTRDFINFO,
                    rdfInfo);
        }
    }
    ...
    public void removeContextRdfInfo() throws LDAPException {
        if (this.attributeExists(CollaborillaObjectConstants.CONTEXTRDFINFO)) {
            this.removeAttribute(CollaborillaObjectConstants.CONTEXTRDFINFO,
                    this.getContextRdfInfo());
        }
    }
    ...
    public String getContainerRdfInfo() throws LDAPException {
        if (this.attributeExists(CollaborillaObjectConstants.CONTAINERRDFINFO)) {
            return this
                    .readAttribute(CollaborillaObjectConstants.CONTAINERRDFINFO)[0];
        } else {
            return null;
        }
    }
     ...
     * @throws LDAPException
     */
    public void setContainerRdfInfo(String rdfLocationInfo)
            throws LDAPException {
        if (this.attributeExists(CollaborillaObjectConstants.CONTAINERRDFINFO)) {
            this.resetAttribute(CollaborillaObjectConstants.CONTAINERRDFINFO,
                    rdfLocationInfo);
        } else {
            this.addAttribute(CollaborillaObjectConstants.CONTAINERRDFINFO,
                    rdfLocationInfo);
        }
    }
    ...
    public void removeContainerRdfInfo() throws LDAPException {
        if (this.attributeExists(CollaborillaObjectConstants.CONTAINERRDFINFO)) {
            this.removeAttribute(CollaborillaObjectConstants.CONTAINERRDFINFO,
                    this.getContainerRdfInfo());
        }
    }
    ...
    public void removeDescription() throws LDAPException {
        if (this.attributeExists(CollaborillaObjectConstants.DESCRIPTION)) {
            this.removeAttribute(CollaborillaObjectConstants.DESCRIPTION, this
                    .getDescription());
        }
    }
