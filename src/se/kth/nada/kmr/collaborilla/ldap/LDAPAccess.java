...
public class LDAPAccess {
     ...
     */
    public LDAPAccess(String host, String loginDN, String password, int port,
            int binding) {
        this.ldapHost = host;
        this.ldapLoginDN = loginDN;
        this.ldapPassword = password;
    ...
    public void bind() throws LDAPException, IllegalArgumentException {
        if (!this.ldapConnection.isBound()) {
            ...
            case BIND_MD5:
                ...
                break;
            default:
                throw new IllegalArgumentException("Unknown binding method: "
                        + String.valueOf(this.bindingMethod));
            }
        }
    }
    ...
    private void bindSimple() throws LDAPException {
        ...
        try {
            this.ldapConnection.bind(this.ldapVersion, this.ldapLoginDN,
                    this.ldapPassword.getBytes("UTF8"));
    ...
    public void bindMD5() throws LDAPException {
        ...
        try {
            this.ldapConnection.bind(this.ldapLoginDN, "dn: "
                    + this.ldapLoginDN, mechanisms, null,
                    new BindCallbackHandler(this.ldapPassword));
        } catch (NullPointerException ne) {
            throw new LDAPException("Coult not bind to LDAP server",
                    LDAPException.AUTH_UNKNOWN, (String) null, ne);
        }
    }
