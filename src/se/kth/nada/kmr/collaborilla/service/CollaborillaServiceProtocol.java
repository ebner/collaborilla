...
public class CollaborillaServiceProtocol {
    ...
    public CollaborillaServiceResponse requestDistributor(String request) {
        ...
        if (paramCount == 1) {
            if (command[0]
                    .equalsIgnoreCase(CollaborillaServiceCommands.CMD_HELP)) {
        ...
        if (paramCount < 2) {
            return new CollaborillaServiceResponse(
                    CollaborillaServiceStatus.SC_BAD_REQUEST);
        }
        ...
        if (command[0].equalsIgnoreCase(CollaborillaServiceCommands.CMD_URI)) {
            if ((paramCount >= 3)
                    && command[1]
                            .equalsIgnoreCase(CollaborillaServiceCommands.CMD_URI_NEW)) {
        ...
        if (command[0].equalsIgnoreCase(CollaborillaServiceCommands.CMD_GET)) {
            if (command[1]
                    .equalsIgnoreCase(CollaborillaServiceCommands.ATTR_URI_ORIG)) {
                return this.handleGetUriOriginal();
            }

            if (command[1]
                    .equalsIgnoreCase(CollaborillaServiceCommands.ATTR_URI_OTHER)) {
                return this.handleGetUriOther();
            }

            if (command[1]
                    .equalsIgnoreCase(CollaborillaServiceCommands.ATTR_LOCATION)) {
                return this.handleGetLocation();
            }

            if (command[1]
                    .equalsIgnoreCase(CollaborillaServiceCommands.ATTR_ALIGNEDLOCATION)) {
                return this.handleGetAlignedLocation();
            }

            if (command[1]
                    .equalsIgnoreCase(CollaborillaServiceCommands.ATTR_LDIF)) {
                return this.handleGetLdif();
            }

            if (command[1]
                    .equalsIgnoreCase(CollaborillaServiceCommands.ATTR_DESCRIPTION)) {
                return this.handleGetDescription();
            }

            if (command[1]
                    .equalsIgnoreCase(CollaborillaServiceCommands.ATTR_CONTEXT_RDFINFO)) {
                return this.handleGetContextRdfInfo();
            }

            if (command[1]
                    .equalsIgnoreCase(CollaborillaServiceCommands.ATTR_CONTAINER_RDFINFO)) {
                return this.handleGetContainerRdfInfo();
            }

            if ((paramCount >= 3)
                    && command[1]
                            .equalsIgnoreCase(CollaborillaServiceCommands.ATTR_REVISION_INFO)) {
                return this.handleGetRevisionInfo(command[2]);
            }

            if (command[1]
                    .equalsIgnoreCase(CollaborillaServiceCommands.ATTR_REVISION)) {
                return this.handleGetRevision();
            }

            if (command[1]
                    .equalsIgnoreCase(CollaborillaServiceCommands.ATTR_REVISION_COUNT)) {
                return this.handleGetRevisionCount();
            }

            if (command[1]
                    .equalsIgnoreCase(CollaborillaServiceCommands.ATTR_INTERNAL_TIMESTAMP_CREATED)) {
                return this.handleGetTimestampCreated();
            }

            if (command[1]
                    .equalsIgnoreCase(CollaborillaServiceCommands.ATTR_INTERNAL_TIMESTAMP_MODIFIED)) {
        ...
        /* RESTORE REVISION */
        if (command[0]
                .equalsIgnoreCase(CollaborillaServiceCommands.CMD_RESTORE)) {
            if ((paramCount >= 3)
                    && command[1]
                            .equalsIgnoreCase(CollaborillaServiceCommands.ATTR_REVISION)) {
    ...
    private CollaborillaServiceResponse handleUri(String uri) {
        try {
            this.collabObject = new CollaborillaObject(this.ldapConnection,
                    this.serverDN, uri, false);

            return new CollaborillaServiceResponse(
                    CollaborillaServiceStatus.SC_OK);
        } catch (LDAPException e) {
            if (e.getResultCode() == LDAPException.NO_SUCH_OBJECT) {
                return new CollaborillaServiceResponse(
                        CollaborillaServiceStatus.SC_NO_SUCH_OBJECT);
            }

            this.log.write(e.toString());
            return new CollaborillaServiceResponse(
                    CollaborillaServiceStatus.SC_INTERNAL_ERROR);
        }
    }
    ...
    private CollaborillaServiceResponse handleGetUriOriginal() {
        ...
        if (uris == null) {
            return new CollaborillaServiceResponse(
                    CollaborillaServiceStatus.SC_NO_SUCH_ATTRIBUTE);
        }
    ...
    private CollaborillaServiceResponse handleSetContainerRdfInfo(
            String rdfLocationInfo) {
        try {
    ...
    private CollaborillaServiceResponse handleModUriOrig(String oldUri,
            String newUri) {
        try {
            ...
            if (e.getResultCode() == LDAPException.NO_SUCH_ATTRIBUTE) {
                return new CollaborillaServiceResponse(
                        CollaborillaServiceStatus.SC_NO_SUCH_VALUE);
            }
    ...
    private CollaborillaServiceResponse handleModUriOther(String oldUri,
            String newUri) {
        try {
    ...
    private CollaborillaServiceResponse handleModLocation(String oldUrl,
            String newUrl) {
        try {
    ...
    private CollaborillaServiceResponse handleAddUriOrig(String uri) {
        try {
            ...
            if (e.getResultCode() == LDAPException.ATTRIBUTE_OR_VALUE_EXISTS) {
                return new CollaborillaServiceResponse(
                        CollaborillaServiceStatus.SC_ATTRIBUTE_OR_VALUE_EXISTS);
            }
